...
export const addUpdatePrinciple = async (data, id) => {
  ...
  const messages = {
    question: {
      [ValidationTypes.REQUIRED]: "Please enter question."
    },
    answer: {
      [ValidationTypes.REQUIRED]: "Please enter answer."
    }
  };
  let isValid = true;
  ...
  const res = await new ApiHelper().FetchFromServer(
    ...
    id
      ? ApiRoutes.UPDATE_PRINCIPLE_DETAILS.method
      : ApiRoutes.ADD_PRINCIPLE.method,
    id
      ? ApiRoutes.UPDATE_PRINCIPLE_DETAILS
      : ApiRoutes.ADD_PRINCIPLE.authenticate,
    undefined,
    data
  );
  if (!res.isError) {
